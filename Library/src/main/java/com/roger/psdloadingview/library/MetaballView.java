...
public class MetaballView extends View {
    ...
    private final float SCALE_RATE = 0.0f;
    ...
    private void metaball(Canvas canvas, int j, int i, float v, float handle_len_rate, float maxDistance) {
        ...
        float angle2a = (float) (angle1 + Math.PI - u2 -
                (Math.PI - u2 - angle2) * v);
        float angle2b = (float) (angle1 - Math.PI + u2 +
                (Math.PI - u2 - angle2) * v);
        ...
        float[] p1a = new float[] { p1a1[0] + center1[0],
                p1a1[1] + center1[1] };
        float[] p1b = new float[] { p1b1[0] + center1[0],
                p1b1[1] + center1[1] };
        float[] p2a = new float[] { p2a1[0] + center2[0],
                p2a1[1] + center2[1] };
        float[] p2b = new float[] { p2b1[0] + center2[0],
                p2b1[1] + center2[1] };
        ...
        float d2 = Math.min(v * handle_len_rate,
                getLength(p1_p2) / totalRadius);
    ...
    @Override protected void onDraw(Canvas canvas) {
        ...
        for (int i = 1, l = circlePaths.size(); i < l; i++) {
            metaball(canvas, i, 0, 0.6f, handle_len_rate, radius * 2f);
        }
    }
